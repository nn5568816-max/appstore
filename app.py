...
# --- Secret key -------------------------------------------------------
# Never hardcode this in source. If SECRET_KEY isn't set in the
# environment, we generate a random one at startup so the app still
# runs, but that means sessions won't survive a restart — set a real
# fixed value via environment variable for production.
app.secret_key = os.environ.get("SECRET_KEY") or secrets.token_hex(32)
if not os.environ.get("SECRET_KEY"):
    print("[WARN] SECRET_KEY not set — using a random one-time key. "
