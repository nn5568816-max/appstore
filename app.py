...
.cards-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(132px,1fr));gap:10px;}
...
.thumb{width:72%;aspect-ratio:1/1;margin:0 auto 7px;border-radius:18px;overflow:hidden;display:grid;
  place-items:center;font-size:30px;background:radial-gradient(circle,#3a2f6b,#150f2e);}
.thumb img{width:100%;height:100%;object-fit:cover;}
.card h3{font-size:11px;margin:0 0 3px;min-height:28px;}
.card .rating{font-size:10px;color:var(--yellow);}
.card .filesize{font-size:9px;color:var(--muted);margin-top:2px;}
...
.small-btn{flex:1;padding:6px 4px;border-radius:7px;border:1px solid #4a3b7a;background:#1a1338;
  color:#fff;font-size:10px;font-weight:800;cursor:pointer;text-decoration:none;display:inline-block;transition:transform .16s ease,box-shadow .16s ease,filter .16s ease;}
.install{background:linear-gradient(135deg,#00d4ff,#7c4dff 52%,#ff2d92);border-color:transparent;box-shadow:0 4px 12px #8a2be644;}
.install:hover{transform:translateY(-1px);filter:saturate(1.15);box-shadow:0 7px 16px #8a2be655;}
...
/* Mobile phone screen: tall layout with comfortable touch targets */
html, body{min-height:100%;width:100%;}
body{min-height:100vh;min-height:100dvh;overflow-x:hidden;}
.store-header{padding:12px 12px;}
.content{width:100%;max-width:100%;padding:14px 10px 90px;}
.search-row input{height:46px;font-size:14px;}
.tabs{gap:7px;}
.tabs a{padding:9px 11px;}
.cards-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;}
.card{min-width:0;padding:7px 5px;}
.fab{right:14px;bottom:calc(14px + env(safe-area-inset-bottom));width:52px;height:52px;}
.modal{padding:12px;align-items:center;}
.modal-card{width:min(100%,430px);max-height:calc(100dvh - 24px);padding:18px;border-radius:18px;}
.form-row input,.form-row textarea,.form-row select{font-size:16px;min-height:44px;}
.btn,.cancel{min-height:44px;}

@media (max-width:520px){.content{padding:14px 10px 10px}.cards-grid{grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.card{padding:7px 5px}.thumb{width:68%}.card h3{font-size:10px;min-height:25px}.card .rating{font-size:9px}.card .filesize{font-size:8px}.small-btn{padding:6px 2px;font-size:9px}}
@media (min-width:521px) and (max-width:850px){.cards-grid{grid-template-columns:repeat(4,minmax(0,1fr))}}
